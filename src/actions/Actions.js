...
export const action = (payload, updateState) => dispatch => {
  console.log("action url-> action dispatched...->" + payload.type);
  ...
  try {
    if (typeof payload === "string" && typeof updateState === "object") {
      ...
      let url =
        typeof actionTypes[type].url === "function"
          ? actionTypes[type].url(payload.params)
          : actionTypes[type].url || "";
      let requestType = actionTypes[type].requestType || "";
      let processingActionType = actionTypes[type].processing || "";
      let processingExtraActions =
        actionTypes[type].processingExtraActions || "";
      ...
      return req(url, payload, requestType.toLowerCase())
        .then(res => {
          console.log("**********************");
          ...
          } else if (res.statusCode === 401 && res.body && res.body.data) {
